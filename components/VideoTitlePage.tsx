...
// import screenfull from 'screenfull'; // Импорт screenfull
...
//   const playerContainerRef = useRef(null); // Реф для контейнера плеера
...
//       screenfull.toggle(playerContainerRef.current); // Переключение полноэкранного режима через screenfull
//     }
//   };
...
const MultiEpisodeVideoPlayer = ({ episodes }) => {
  ...
  return (
    <div className="relative p-2" ref={playerContainerRef}>
      <div
        className={`flex flex-col items-center justify-center gap-2 sm:flex-row sm:gap-4 ${isFullscreen ? 'absolute top-0 left-0 w-full z-50 bg-black bg-opacity-50' : ''}`}
      >
        <h2 className="text-2xl font-bold">{selectedEpisode.name || 'Без названия'}</h2>
      ...
      <div className="relative">
        <div className="w-full h-0 pb-[56.25%]">
          ...
          {/* Таймлайн */}
          <div
            // className="absolute bottom-7 w-full px-6 z-30"
            className={`${isFullscreen && playing ? 'invisible pointer-events-none' : 'absolute bottom-7 w-full px-6 z-30'}`}
          >
            <ReactSlider
          ...
          {/* Панель управления */}
          <div className="absolute bottom-0 left-0 right-0 p-2 bg-black bg-opacity-50 flex justify-between items-center transition-opacity duration-300 hover:opacity-100 opacity-100">
            {/* Кнопка воспроизведения/паузы */}
            <button
              className="bg-slate-700 text-white px-3 py-1 rounded hover:bg-slate-800 transition-colors"
              onClick={togglePlayPause}
